...
def convert_temp(val, unit, attr, req):
    ...
    try:
        if unit == "°K":
            ...
            raise UnitError(f"Invalid unit request: {unit} in attribute {attr} in {req}. Accepted only: °K, °F, or °C as temperature units for calculation")
...
def convert_vol_flow_rate(val, unit, attr, req):
    ...
    try:
        if unit == "GPM":
            ...
            raise UnitError(f"Invalid unit request: {unit} in attribute: {attr} in {req}. Accepted only: GPM, CFM, or m³/hr as volume flow units for calculation")
    except UnitError as ue:
...
def convert_mass_flow_rate(val, unit, attr, req):
    ...
    try:
        if unit == "kg/hr":
            ...
            raise UnitError(f"Invalid unit request: {unit} in attribute: {attr} in {req}. Accepted only: kg/hr, kg/s as mass flow units for calculation")
    except UnitError as ue:
...
def convert_pressure(val, unit, attr, req):
    ...
    try:
        if unit == "kgf/cm²":
            ...
            raise UnitError(f"Invalid unit request: {unit} in attribute: {attr} in {req}. Accepted only: kgf/cm², PSI, or bar as pressure units for calculation")
    except UnitError as ue:
...
def fetch_values(body, attri, req):
    ...
    """
    try:
        value = body["parameters"][attri]
        if isinstance(value, (int, float)):
            return value
        else:
            raise DatatypeError(f"Invalid datatype {type(value)}. Accepted only: int/float as datatype under parameters for attribute: {attri} in {req}")
